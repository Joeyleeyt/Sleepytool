...
import { transitionOverlapS } from './transitions.js';
...
export async function buildTimeline(projectId: string): Promise<Timeline> {
  ...
  for (let i = 0; i < shots.length; i++) {
    ...
    const overlapOut = transitionOverlapS((shot.transitionOut ?? 'cut') as never);
